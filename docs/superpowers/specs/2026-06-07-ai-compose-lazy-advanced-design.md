...
- `presentation/ai-prompt/routes.ts` + `presentation/agent/apiRoutes.ts`: enqueue-схема
  допускает новый mode; для `compose-advanced` потолок `text` поднят до 30000 символов (JSON
  сегментов; под колонку `input_text` TEXT ≤ 65535 байт), 5000-лимит сохранён для свободного
  текста через `superRefine`.
...
- `db/065_ai_prompt_compose_advanced.sql`: `ALTER TABLE ai_prompt_jobs MODIFY COLUMN mode
  ...
  INSTANT-операция (без блокировки/перестройки). Применяется на проде через `deploy.mjs`
  (шаг `migrate.mjs`).
