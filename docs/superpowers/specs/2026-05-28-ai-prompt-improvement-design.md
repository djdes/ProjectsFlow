...
loop:
  ...
  # 1. AI-prompt-job'ы — приоритет (короткие, юзер ждёт).
  ai_jobs = pf_list_pending_ai_prompt_jobs(limit=10)
  for job in ai_jobs:
    claimed = pf_claim_ai_prompt_job(job.id)    # 409 → skip
    if claimed:
      handle_ai_prompt_job(claimed)
      has_work = true
      break

  if has_work: continue

  # 2. Затем — обычные kanban agent_jobs (как раньше).
  for p in projects:
    if p.queuedAgentJobCount > 0:
      ...                                        # см. ralph-dispatcher-guide.md
      has_work = true
      break

  if has_work: continue

  # 3. TODO-задачи без явной очереди.
  ...

  sleep(60s)   # минута между тиками — для AI-job'ов хватает; юзер ждёт ≤25s, поэтому
               # в худшем случае его запрос подождёт следующего тика и отобьётся
               # таймаутом — это нормальная honest UX, см. fallback.
```

**Латентность.** Best-case: Ralph как раз поллит — задержка ≈3–8 сек (claim + Claude inference + complete). Worst-case (в пределах TTL долгожданий): юзер успевает в окно <25 сек одного тика. Хуже — таймаут на фронте.

### 8.2 `handle_ai_prompt_job` — псевдокод

```pseudo
def handle_ai_prompt_job(job):
  prompt = SYSTEM_PROMPT  # стабильный, кэшируемый
  user_msg = ""
  if job.kbContext:
    user_msg += "Контекст проекта (база знаний):\n" + job.kbContext + "\n\n---\n\n"
  user_msg += "Черновик задачи:\n" + job.inputText
  ...
  try:
    ...
    pf_complete_ai_prompt_job(job.id, ok=False, error=short_error_string(e))
```
