...
# Итерация 2 (фидбек юзера, 2026-07-11)

1. **Фикс пикера проектов:** эмодзи-иконка (`ProjectIconView` с `size-full`) растягивалась
   на всю кнопку → огромный пробел, обрезанное имя. Иконка теперь в фикс-квадрате
   `grid size-4 place-items-center`.
2. **Любой дроп доски-карточки в верхний канбан = «мне делегировалось»:** общий шаг
   `ensureSelfDelegated` (нет делегации → self-delegate accepted; чужую не трогаем) добавлен
   к дропам по дедлайну и приоритету (включая дроп «в тот же бакет/приоритет» — quiet-claim);
   в сортировке «по дате создания» фантомная колонка стала droppable «Забрать себе»
   (свойство не меняется — дату не изменить, о чём и написано в hint; задача поручается мне
   и сама встаёт по дате).
3. **Дроп пилюли блока на колонку нижней доски = снять делегацию + статус колонки:**
   моя исходящая → `withdraw`; поручена мне → **новый бэк `RelinquishTaskDelegation`**
   (делегат складывает с себя: pending|accepted|pending_invite → withdrawn, без уведомлений;
   роут `POST /api/delegations/:id/relinquish`, клиентский `relinquish()`). Для задач своего
   инбокса дополнительно `board.moveTask` в статус колонки (в начало видимой порции, как
   «Перенести» из TaskDrawer); задачи чужих проектов — только снятие делегации. Дроп на
   карточку доски = её колонка.
