...
### Чат-виджет: поддержка (`SUPPORT_TELEGRAM_CHAT_ID`)

Виджет помощи снизу справа (вкладка «Связаться с поддержкой») складывает обращения
в таблицу `support_tickets` и доставляет их в Telegram. Куда именно — задаёт
переменная `SUPPORT_TELEGRAM_CHAT_ID`:

- **Задана** (число; для групп отрицательное, напр. `-1001234567890`) — обращения
  уходят прямо в этот чат/группу. Бот `@projectsflow_bot` должен в нём состоять.
- **Пусто/не задана** (текущий дефолт) — **fallback**: обращение рассылается всем
  админам в их личный Telegram (через ту же привязку, что и остальные уведомления).

Тикет в БД (`support_tickets`) сохраняется в любом случае — доставка в Telegram
best-effort и не роняет отправку. Чтобы узнать `chat_id` группы: добавь бота в
группу и посмотри `getUpdates`, либо перешли сообщение из неё боту `@getidsbot`.
