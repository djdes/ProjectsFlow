...
| `pf_move_task` | Перенести задачу на статус `backlog` / `todo` / `in_progress` / `awaiting_clarification` / `done` (в конец колонки). `awaiting_clarification` = «ждёт человека» (ответ на ralph-question, разбор retry-fail); сервер автоматически возвращает в `in_progress` когда приходит коммент с маркером `<!-- ralph-answer ` или `<!-- ralph-grillme-summary `. |
...
| Tool | Описание |
|---|---|
| `pf_publish_site` | Опубликовать собранную статику проекта — **единственный** способ, которым сборка попадает в превью и на публичный поддомен (`npm run build` сам по себе на платформе не виден). Читает каталог сборки с диска рекурсивно, сохраняя относительные пути, и заливает его одним multipart-запросом; прошлый артефакт заменяется целиком. `dir` — абсолютный или относительно рабочей папки, по умолчанию первый существующий из `dist` / `build` / `out`. Лимиты: 2000 файлов, 25 МБ на файл, 100 МБ суммарно (зеркалят серверные). Требует роль диспетчера проекта. Возвращает `slug`, `url`, `publishedAt`, `fileCount`, а также `warnings` и (если не `verify: false`) `checks` — HTTP-проверку опубликованного адреса. **Читай `warnings` перед тем как рапортовать «готово»**: там всплывает отсутствие `index.html` и недоступность залитых файлов. |
...
| `pf_get_project_git_token` | Делегированный GitHub OAuth-токен для git-операций (clone/push/PR). **v0.15: per-member opt-in** — любой член проекта включает СВОЮ делегацию независимо. Сервер выбирает первого подходящего в порядке owner→displayName ASC (caller-диспетчер исключается). Response: `source: 'owner_delegation' \| 'member_delegation'`, `grantedByDisplayName`. URL: `https://x-access-token:<token>@github.com/owner/repo.git`. **403** `not_dispatcher` / `delegation_disabled` (никто не включил) / `no_eligible_grantor` (включили, но без GH — body содержит `candidatesChecked`). Токен НЕ персистить, НЕ логировать. Каждое обращение пишется в audit-log владельца. |
