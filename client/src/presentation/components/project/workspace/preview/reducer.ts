...
export type PreviewEditorAction =
  ...
  | { type: 'PATCH_START' }
  | { type: 'PATCH_SUCCESS'; revision: number }
  | { type: 'PATCH_ERROR'; message: string }
  | { type: 'HISTORY'; revision: number; undoDepth: number; redoDepth: number }
...
export function createPreviewEditorState(path = '/'): PreviewEditorState {
  return {
    ...
    saveStatus: 'clean', revision: 0, undoDepth: 0, redoDepth: 0,
...
export function previewEditorReducer(state: PreviewEditorState, action: PreviewEditorAction): PreviewEditorState {
  switch (action.type) {
    ...
    case 'APPLY_PATH': return { ...state, path: action.path, draftPath: action.path, routeMenuOpen: false, hovered: null, selected: null };
    ...
    case 'SESSION_READY': return { ...state, revision: action.revision, saveStatus: 'clean', undoDepth: 0, redoDepth: 0 };
    ...
    case 'PATCH_SUCCESS': return { ...state, saveStatus: 'clean', revision: action.revision, undoDepth: state.undoDepth + 1, redoDepth: 0, bridgeError: null };
    case 'PATCH_ERROR': return { ...state, saveStatus: 'error', bridgeError: action.message };
    case 'HISTORY': return { ...state, saveStatus: 'clean', revision: action.revision, undoDepth: action.undoDepth, redoDepth: action.redoDepth };
