...
function withMode(mode: AiComposerMode, onChange: (next: AiComposerMode) => void): ComposerProps {
  return {
...
// «Размышление» — про то, чтобы НЕ делать, поэтому оно не зависит ни от выделенной зоны,
// ни от режима правки: подумать можно всегда. Раньше тумблер гас без выделения.
test('тумблер «Размышление» доступен и без выделенной зоны', async () => {
  const view = await render(withMode('build', () => undefined));
  assert.equal(view.modeToggle.disabled, false);
  assert.match(view.host.textContent ?? '', /Размышление/);
  await view.unmount();
});

test('по умолчанию «Размышление» выключено — агент сразу делает', async () => {
  const view = await render(withMode('build', () => undefined));
  assert.equal(view.modeToggle.getAttribute('aria-pressed'), 'false');
  // Без выделенной зоны обещаем работу по проекту, а не правку несуществующей зоны.
  assert.match(view.host.textContent ?? '', /Что сделать в проекте/);
  await view.unmount();
});

test('тумблер меняет только плейсхолдер и нажатое состояние', async () => {
  const changed: AiComposerMode[] = [];
  const view = await render(withMode('build', (next) => changed.push(next)));

  await view.click(view.modeToggle);
  assert.deepEqual(changed, ['discuss']);

  await view.rerender(withMode('discuss', (next) => changed.push(next)));
  assert.match(view.host.textContent ?? '', /О чём подумаем/);
