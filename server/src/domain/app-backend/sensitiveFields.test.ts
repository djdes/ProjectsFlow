...
test('sensitiveColumns = объединение явного флага и эвристики; сильнейшая защита побеждает', () => {
  const map = sensitiveColumns([
    { name: 'title', type: 'text' },                         // ничего
    { name: 'note', type: 'text', sensitive: 'secret' },     // только флаг
    { name: 'email', type: 'text' },                         // только эвристика (pii)
    { name: 'nickname', type: 'text', sensitive: 'pii' },    // только флаг (pii)
    { name: 'api_key', type: 'text', sensitive: 'pii' },     // эвристика secret + флаг pii → secret
  ]);
  assert.equal(map.get('title'), undefined);
  assert.equal(map.get('note'), 'secret');
  assert.equal(map.get('email'), 'pii');
  assert.equal(map.get('nickname'), 'pii');
  assert.equal(map.get('api_key'), 'secret', 'сильнейшая защита побеждает');
});
