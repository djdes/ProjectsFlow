...
import { DrizzleTaskTemplateRepository } from './infrastructure/repositories/DrizzleTaskTemplateRepository.js';
import { DrizzleTaskPropertyRepository } from './infrastructure/repositories/DrizzleTaskPropertyRepository.js';
...
import { GetMyCompletedStats } from './application/stats/GetMyCompletedStats.js';
...
// Второй случай — job, который вообще никто не забрал (раннер выключен или у проекта нет
// живого диспетчера). Он опаснее running: публикация черновиков отказывается стартовать,
// пока висит очередь, поэтому один непринятый job навсегда блокирует сохранение правок
// в проект. Порог больше — занятый воркер имеет право подумать.
const sweepSiteEditorJobs = (): void => {
  void siteEditorService.sweepStaleRunningJobs().catch(() => {});
  void siteEditorService.sweepStaleQueuedJobs().catch(() => {});
};
sweepSiteEditorJobs();
setInterval(sweepSiteEditorJobs, 5 * 60_000).unref();
...
const { app, devProxyUpgrade } = createApp({
  ...
  meStats: {
    ...
    getMyCompletedStats: new GetMyCompletedStats({
      tasks: taskRepo,
      activity: activityRepo,
      workspaces: workspaceRepo,
    }),
