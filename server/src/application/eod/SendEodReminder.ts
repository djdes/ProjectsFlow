...
export class SendEodReminder {
  ...
  async execute(projectId: string): Promise<void> {
    ...
    // Открытые задачи, сгруппированные по единственному ответственному.
    const byAssignee = new Map<string, Task[]>();
    for (const task of openTasks) {
      const arr = byAssignee.get(task.assignee.userId) ?? [];
      arr.push(task);
      byAssignee.set(task.assignee.userId, arr);
    }
        ...
        if (chatId !== null) {
          await this.deps.telegramClient.sendMessage({
            ...
            text:
              ...
              `Если что-то не движется — спросите ответственного или предложите помощь. ` +
