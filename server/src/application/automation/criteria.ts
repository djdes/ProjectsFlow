...
// одно продуманное описание задачи на русском, без преамбулы, ≤800 символов, с учётом
// «уточнения» юзера. Порядок массива важен: round-robin идёт по нему.
...
const BASE_TAIL =
  ...
  'Без преамбулы вроде «Вот задача», без markdown-заголовков. Объём — до 800 символов.';
...
export const AUTOMATION_CRITERIA: ReadonlyArray<AutomationCriterionDef> = [
  {
    ...
    defaultSystemPrompt:
      ...
      'Фича должна быть достаточно мелкой, чтобы её можно было сделать за один заход, но при ' +
      'этом полезной. ' +
      BASE_TAIL,
  },
  {
    ...
    defaultSystemPrompt:
      ...
      'текущего состояния экрана: сделать скриншот через браузер (Playwright), найти конкретные ' +
      'визуальные проблемы (отступы, контраст, выравнивание, адаптив, состояния hover/focus) и ' +
      'предложить точечные правки. Никакой переделки архитектуры — только вёрстка и стили. ' +
      BASE_TAIL,
  },
  {
    key: 'refactor',
    ...
    defaultSystemPrompt:
      'Ты ставишь задачи на рефакторинг backend-кода без изменения поведения. Выбери один ' +
      'небольшой участок (дублирование, длинная функция, слабая типизация, мёртвый код) и ' +
      'опиши аккуратную правку. Задача ОБЯЗАНА включать прогон линтера и тестов и требование ' +
      'оставить их зелёными. Никаких изменений API и поведения — только чистота кода. ' +
      BASE_TAIL,
  },
  {
    key: 'security',
