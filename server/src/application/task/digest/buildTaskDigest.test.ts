...
test('renderDigestRich: one mobile-friendly column with meta and action below the task', () => {
  ...
  assert.ok(rich.includes('<tr><th>Задача</th></tr>'));
  assert.ok(!rich.includes('<th>Кто</th>'));
  assert.ok(!rich.includes('<th>Дедлайн</th>'));
  assert.ok(rich.includes(
    '<a href="https://projectsflow.ru/projects/p1?task=t1"><b>Проверить отчёт</b></a>',
  ));
  assert.ok(rich.includes('<br>👤 Я · ⏰ осталось 3 дня'));
  assert.ok(rich.includes(
