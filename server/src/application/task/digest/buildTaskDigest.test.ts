...
test('renderDigestTelegram: task names remain plain text and actions live in native buttons', () => {
...
test('renderDigestRich: collapsed two-column table is narrow-screen safe', () => {
  ...
  assert.ok(rich.includes('<table'));
  assert.ok(rich.includes('table-layout:fixed'));
  assert.ok(rich.includes('width:72%'));
  assert.ok(rich.includes('<details><summary>Показать задачи (1)</summary>'));
  assert.ok(rich.endsWith('</details>'));
  assert.ok(!rich.includes('<th></th>'));
  assert.ok(rich.includes('<b>Проверить отчёт</b>'));
  assert.ok(!rich.includes('projects/p1?task=t1'));
  assert.ok(rich.includes('<small>👤 Я</small>'));
  assert.ok(rich.includes('>осталось 3 дня</td>'));
});

test('digestTaskKeyboard: each table row gets compact complete and open buttons', () => {
  const model = buildDigestModel(
    [task({ id: '11111111-1111-4111-8111-111111111111', description: 'Проверить отчёт' })],
    baseOpts,
  );
  assert.deepEqual(digestTaskKeyboard(model).inline_keyboard, [
    [
      { text: '✓ 1', callback_data: 'dg:11111111-1111-4111-8111-111111111111' },
      { text: '↗ 1', url: 'https://projectsflow.ru/projects/p1?task=11111111-1111-4111-8111-111111111111' },
    ],
  ]);
});
...
test('renderDigestTelegram: no inline action links are mixed into table fallback', () => {
