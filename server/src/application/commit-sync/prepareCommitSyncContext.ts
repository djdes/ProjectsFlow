...
export function prepareCommitSyncContext(params: {
  ...
  let context =
    ...
    `1. Сопоставь git-коммиты с открытыми задачами ПО СМЫСЛУ. Это не поиск по id: ` +
    `содержание коммита должно действительно закрывать или продвигать задачу. Порог ${thresholdHours} ч ` +
    `применяет СЕРВЕР — тебе не нужно выбирать статус задачи.\n\n` +
    `2. Сделай короткий содержательный обзор коммитов за последние ${reviewWindowHours} ч. ` +
    `Выбери только значимые изменения: иногда это один коммит, иногда несколько. Не перечисляй ` +
    `служебные merge/format/chore-коммиты без полезного вывода. Для сильного изменения используй verdict=good, ` +
    ...
    `Выбери для них один, несколько или ни одного значимого коммита; итоговая запись summary нужна всегда. ` +
    `Только очевидные смысловые совпадения. ` +
    `Не выбирай in_progress/done — это делает сервер.`;
