...
// HTML-письмо с яркой CTA-кнопкой «Принять приглашение». Inline-CSS — почтовые
// клиенты игнорируют <style>/внешние стили. Чистая функция без I/O — потому в application.
export function renderInviteEmail(input: InviteEmailInput): EmailMessage {
  const subject = `Вас пригласили в проект «${input.projectName}» в ProjectsFlow`;

  const text = [
    `${input.actorDisplayName} приглашает вас в проект «${input.projectName}» как ${roleLabel[input.role]}.`,
    '',
    ...
    <tr><td align="center">
      <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
        ...
        <tr><td style="padding:8px 32px 0;">
          <h1 style="margin:0 0 12px;font-size:20px;line-height:1.3;color:#0f172a;">Приглашение в проект</h1>
          <p style="margin:0 0 8px;font-size:15px;line-height:1.5;color:#334155;">
            <strong style="color:#0f172a;">${input.actorDisplayName}</strong> приглашает вас присоединиться к проекту
            <strong style="color:#0f172a;">«${input.projectName}»</strong> как <strong>${roleLabel[input.role]}</strong>.
          </p>
        </td></tr>
