...
test('EnsureProjectAppRepo: editor подключил готовый репо → делегирует свой токен и создаёт KB', async () => {
  const project = makeProject({
    gitRepoUrl: 'https://github.com/editor/project',
    appRepoFullName: 'editor/project',
  });
  const { deps, calls } = makeDeps({ project, role: 'editor', connected: true });
  const out = await new EnsureProjectAppRepo(deps).execute('p1', 'editor1');
  assert.equal(out.fullName, 'editor/project');
  assert.equal(calls.createRepo.length, 0);
  assert.deepEqual(calls.updates, [{ kbKind: 'local' }]);
  assert.deepEqual(calls.delegationUpserts, [
    { projectId: 'p1', granterUserId: 'editor1', enabled: true },
  ]);
  assert.deepEqual(calls.putFile, ['.github/workflows/pf-build-site.yml', 'index.html']);
});
...
test('EnsureProjectAppRepo: editor без подключённого репо не может создать owner-only app-репо', async () => {
