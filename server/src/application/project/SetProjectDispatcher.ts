...
// Назначить (или снять) Ralph-диспетчера проекта. owner-only (S1): диспетчер получает
// делегированный GitHub-токен владельца, поэтому «кто диспетчер» решает владелец.
// Admin'у admin-bypass позволяет менять диспетчера в любом проекте (admin-панель).
//
