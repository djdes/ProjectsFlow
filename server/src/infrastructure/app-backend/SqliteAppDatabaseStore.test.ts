...
test('select/findOne/update/remove + фильтр/сортировка/лимит', () => {
  ...
  // remove по таблице схемы — мягкое удаление (срез 5): строка выпадает из выборки, но остаётся
  // в файле с меткой deleted_at (findOne её ещё видит, чтобы restore мог проверить владельца).
  assert.equal(s.remove('proj-2', 'posts', 'p1'), 1);
  assert.equal(s.select('proj-2', 'posts', { where: { id: 'p1' } }).length, 0);
  assert.ok(s.findOne('proj-2', 'posts', { id: 'p1' })?.deleted_at, 'метка удаления проставлена');
  // Повторное удаление уже удалённой строки меняет 0 строк.
  assert.equal(s.remove('proj-2', 'posts', 'p1'), 0);
  // restore возвращает строку в выборку.
  assert.equal(s.restore('proj-2', 'posts', 'p1'), 1);
  assert.equal(s.select('proj-2', 'posts', { where: { id: 'p1' } }).length, 1);
});
